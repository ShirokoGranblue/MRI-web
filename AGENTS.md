...
`admin` is presented as the doctor. Doctors can read patient profiles and contraindications and continue to process exam orders, schedules, examinations, image archives, uploads, reports, reviews, and publication. Doctors must not create, edit, or delete patient-owned profiles or contraindications. `PATIENT` users can create and maintain only their profile bound through `patient.account_username`, submit an exam request only for that bound profile, and read only their own exam, image, and report progress through `/me` and `/mine` endpoints. Report text and image content or downloads remain unavailable to the patient until the report is `PUBLISHED`. Preserve the existing exam and report state machines.

Creating or modifying an exam order must save a contraindication risk snapshot. Starting an exam must fetch the current contraindications again; unavailable patient data blocks the transition, and `HIGH` risk requires an explicit confirmation recorded with the gateway-authenticated doctor and time. Schedule writes must go through `ScheduleService`, use half-open time ranges, and reject overlaps for either the same scanner room or the same nonblank technologist. Downloads must use gateway-authenticated identity, enforce `downloadEnabled` in the backend, and create audit records only after the file or ZIP has been produced successfully.
...
Do not commit secrets, generated logs, `target/`, `node_modules/`, frontend `dist/`, or runtime files under `storage/mri-images`. Keep local service changes in `application.yml` explicit, and document non-default ports or credentials in the PR description. Apply both idempotent migrations in `docker/mysql/init/02-patient-account-migration.sql` and `docker/mysql/init/03-mri-enhancement-migration.sql` for existing volumes. Keep system Chinese seed values encoded with explicit UTF-8-safe SQL expressions so Windows PowerShell piping cannot corrupt role or display names.
