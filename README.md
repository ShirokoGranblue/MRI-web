...
- **检查安全联动**：申请时保存 MRI 禁忌症风险快照，开始检查前重新评估，高风险由当前医生确认并留痕。
- **排程冲突检测**：按检查时长同时检测检查室和技师的时间段重叠，允许首尾相接。
- **受控影像下载**：医生和患者可下载单个影像或 Study ZIP，后端统一执行权限、配置和成功审计。
...
| 检查业务 | 为患者创建申请，查看风险，处理排程、开始、完成或取消检查 | 为本人提交申请，只读查看排程和处理状态 |
| 影像业务 | 创建 Study/Series、上传、预览、下载和管理影像 | 发布前查看归档进度，发布后查看并下载本人影像 |
...
检查申请保存时会读取患者当前禁忌症并形成风险快照。开始检查前再次读取最新禁忌症，避免申请后新增的高风险信息被遗漏；存在高风险时必须由当前医生明确确认。排程包含开始时间和检查时长，按半开区间 `[开始时间, 结束时间)` 检测同一检查室或同一技师的重叠，前一项结束后可立即安排下一项。
...
- 将申请安排到具体 MRI 检查室、时间、检查时长和执行技师，冲突时显示具体占用区间。
- 查看无风险、低风险、高风险或待评估状态；开始检查前复核最新禁忌症，高风险确认后进入检查中。
- 检查完成后创建 Study、Series，并通过 Multipart 上传真实影像到 MinIO。
- 编写诊断报告，执行提交、审核通过、驳回、回到草稿和发布。
- 选择诊断查看、会诊、归档导出或其他用途，下载单个影像或整个 Study ZIP，并查看真实下载记录。
...
- 为本人提交 MRI 检查申请；服务端根据登录用户名强制绑定本人患者档案。
- 只读查看本人检查申请、排程、影像归档和报告审核进度。
- 报告发布前不返回报告正文，也不开放影像 manifest 和文件内容。
- 报告发布后查看完整影像所见、诊断意见和关联影像，并下载单个影像或整个 Study ZIP。
...
基础设施脚本会启动仅绑定本机回环地址的 MySQL、Redis、Nacos 和 MinIO，并依次执行患者账号迁移与 MRI 增强功能迁移；两份迁移均可重复执行。服务启动脚本会在当前进程组内生成临时 JWT 签名密钥，不会把签名密钥写入仓库。
...
```powershell
$env:MRI_JWT_SECRET = "请替换为至少 32 个随机字符且不要提交到仓库"
...
该账号仅用于本机课程演示。网关、Vite 开发服务器和基础设施默认只监听
`127.0.0.1`；不得直接把演示配置暴露到公网。确需从其他主机访问时，应先更换
管理员密码及 MySQL、MinIO 凭据，再显式设置 `MRI_GATEWAY_ADDRESS` 并配置受控入口。
...
| 验证项 | 结果 |
| --- | --- |
| 后端 `mvn clean test` | 103 项通过，0 失败、0 错误、0 跳过 |
| 前端轻量回归 | 13 项通过，0 失败 |
| 前端生产构建 | Vite 8.0.16，1583 个模块，构建成功 |
...
完整证据见[测试执行结果](docs/test-result.md)。
...
- [视频增强功能补充录制说明](docs/video-enhancement-guide.md)
