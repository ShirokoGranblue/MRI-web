...
新增 minio 服务：镜像 `minio/minio:latest`，`server /data --console-address ":9001"`，环境 `MINIO_ROOT_USER=mri / MINIO_ROOT_PASSWORD=mri123456`，端口 `9000:9000`、`9101:9001`，卷 `mri-minio-data:/data`。`01-start-infra.ps1` 显式启动 MinIO。MinIO 控制台 `http://localhost:9101`（mri/mri123456）用于验证已存图像；宿主机 `9001` 保留给认证服务。
